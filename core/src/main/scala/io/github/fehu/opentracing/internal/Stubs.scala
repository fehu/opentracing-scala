...
protected[opentracing] class TracedStub[F[_]](implicit A: Applicative[F], D: Defer[F])
    extends TracedInterfaceStub[F]
       with Traced[F] {
...
protected[opentracing] class TracedInterfaceStub[F[_]](implicit A: Applicative[F]) extends Traced.Interface[F] {
  ...
  def spanResource(op: String, tags: Traced.Tag*): Resource[F, Traced.ActiveSpan] =
    Resource.pure(Traced.ActiveSpan.empty)
...
protected[opentracing] class SpanInterfaceStub[F[_]](unit: F[Unit], pure: cats.Id ~> F)
