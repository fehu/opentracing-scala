...
private[opentracing] trait TracedTInstances extends TracedTLowPriorityInstances1 {
  ...
  implicit def tracedTParallelInstance[F[_]](implicit par: Parallel[F]): Parallel.Aux[TracedT[F, _], TracedTParallelInstance.Par[par.F, _]] =
    new TracedTParallelInstance[F, par.F]()(par)

  implicit def tracedTAsyncInstance[F[_]: Async]: Async[TracedT[F, _]] = new TracedTAsyncInstance[F] { protected val F: Async[F] = Async[F] }
}
...
private[opentracing] class TracedTTracedInstance[F[_]](implicit sync: Sync[F])
  ...
  def withParent(span: ActiveSpan): Traced.Interface[TracedT[F, _]] = withParent0(Option(span).flatMap(_.maybe).map(_.asLeft))
...
private[opentracing] trait TracedTMonadErrorProxy[F[_], E] extends MonadError[TracedT[F, _], E] with TracedTMonadProxy[F] {
...
private[opentracing] trait TracedTMonadCancelProxy[F[_], E] extends MonadCancel[TracedT[F, _], E] with TracedTMonadErrorProxy[F, E] {
...
private[opentracing] trait TracedTSyncProxy[F[_]] extends Sync[TracedT[F, _]]
...
private[opentracing] class TracedTFiber[F[_]: Monad, E, A](f: Fiber[F, E, A], s0: State) extends Fiber[TracedT[F, _], E, A] {
...
private[opentracing] trait TracedTGenSpawnInstance[F[_], E] extends GenSpawn[TracedT[F, _], E] with TracedTMonadCancelProxy[F, E] {
  ...
  def racePair[A, B](fa: TracedT[F, A], fb: TracedT[F, B]): TracedT[F, Either[(Outcome[TracedT[F, _], E, A], Fiber[TracedT[F, _], E, B]), (Fiber[TracedT[F, _], E, A], Outcome[TracedT[F, _], E, B])]] =
    TracedT(
      for {
        s0 <- StateT.get[F, State]
...
private[opentracing] trait TracedTGenConcurrentInstance[F[_], E] extends GenConcurrent[TracedT[F, _], E] with TracedTGenSpawnInstance[F, E] {
...
private[opentracing] trait TracedTGenTemporalInstance[F[_], E] extends GenTemporal[TracedT[F, _], E]
  with TracedTGenConcurrentInstance[F, E]
  with TracedTClockProxy[F]
{
  protected val F: GenTemporal[F, E]
  protected val C: Clock[F] = F
  private implicit def F0: GenTemporal[F, E] = F
...
private[opentracing] class TracedTCont[F[_]: Sync, E, A](c: Cont[TracedT[F, _], E, A], s0: State) extends Cont[F, E, A] {
...
private[opentracing] trait TracedTAsyncInstance[F[_]] extends Async[TracedT[F, _]]
...
object TracedTParallelInstance {
  ...
  object Par {
    ...
    protected[TracedTParallelInstance] def parCommutativeApplicative0[F[_]](implicit A: Applicative[F]): CommutativeApplicative[Par[F, _]] =
